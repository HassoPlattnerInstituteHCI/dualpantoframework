...
class HardStep extends Collider {
  ...
  init() {
    ...
    const array = this.traceableObject.path.map((v) => [
    ...
    if (this.enterStrength > 0) {
      try {
        ...
        console.log(
            ...
            'Problematic mesh:', this.traceableObject.path,
            '\nError:', exception);
        this.enterMesh = new Mesh(this.traceableObject.path);
      }
    } else {
      this.enterMesh = new Mesh(this.traceableObject.path);
    }
    ...
    if (this.leaveStrength > 0) {
      try {
        ...
        console.log(
            ...
            'Problematic mesh:', this.traceableObject.path,
            '\nError:', exception);
        this.leaveMesh = new Mesh(this.traceableObject.path);
      }
    } else {
      this.leaveMesh = new Mesh(this.traceableObject.path);
    }
