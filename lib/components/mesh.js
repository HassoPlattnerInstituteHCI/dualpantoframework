 ...
 */
class Mesh extends TraceableObject {
  /**
  ...
  trace(index = 1) {
    return new Promise((resolve) => {
      ...
      traceScript.push(() => this.hapticObject.device.movePantoTo(index,
          this.path[0].sum(this.hapticObject.position)));
      for (let i = 1; i < this.path.length; i++) {
        traceScript.push(() => this.hapticObject.device.movePantoTo(index,
            this.path[i].sum(this.hapticObject.position), 50));
      }
      traceScript.push(() => this.hapticObject.device.movePantoTo(index,
          this.path[0].sum(this.hapticObject.position), 50).then(
          () => resolve(resolve)));
