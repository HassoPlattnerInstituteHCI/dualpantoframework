...
class Collider extends Component {
  ...
  create() {
    ...
    this.sendCreate(data(), handleIdx);

    while (meshCopy.length > 0) {
      segment = meshCopy.splice(0, maxPointsInPacket);
      this.sendAdd(data(), handleIdx);
    }

    this.sendEnable(handleIdx);
  }
