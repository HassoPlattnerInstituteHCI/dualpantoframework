...
class Device extends EventEmitter {
  ...
  /**
   * Enqueues a packet defining a new obstacle to be send via the serial connection to the panto.
  ...
  /**
   * Enqueues a packet deleting an existing obstacle to be send via the serial connection to the panto.
   * @param {number} id - the obstacle's unique id
   * @param {number} [index =-1] - index of affected handle with -1 meaning both
   */
  sendDeleteObstacle(id, index) {
    this.send({
      messageType: 0xA1,
      data: {
        id: id,
  ...
  /**
   * Enqueues a packet enabling an existing obstacle to be send via the serial connection to the panto.
   * @param {number} id - the obstacle's unique id
   * @param {number} [index =-1] - index of affected handle with -1 meaning both
   */
  ...
  /**
   * Enqueues a packet disabling an existing obstacle to be send via the serial connection to the panto.
   * @param {number} id - the obstacle's unique id
   * @param {number} [index =-1] - index of affected handle with -1 meaning both
   */
  ...
  /**
   * Delete obstacles for handles
   * @param {number} id - the id of the obstacle that should be deleted
   * @param {number} [index =-1] - index of affected handle with -1 meaning both
   */
  deleteObstacle(id, index = -1) {
    this.usedObstacleIds.delete(id);
    this.sendDeleteObstacle(id, index);
    this.emit("deleteObstacle", index, id);
  }
  ...
  movePantoTo(
    ...
    return new Promise(resolve => {
      const distance = target
        .difference(this.lastKnownPositions[index])
        .length();
