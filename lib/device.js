...
const serial = require('../build/Release/serial'),
      ...
      TWEEN_INTERVAL = 30,
      MoveableObject = require('./moveableObject'),
...
class Device extends EventEmitter {
    ...
    poll() {
        ...
        if(packet.length == 16)
            ...
            for(let i = 0; i < 2; ++i) {
                ...
                if(this.lastKnownPositions[i] &&
                   newPosition.difference(this.lastKnownPositions[i]).length() <= 0.0 &&
                   Math.abs(newPosition.r-this.lastKnownPositions[i].r) <= 0.0)
                    continue;
                this.lastKnownPositions[i] = newPosition;
                this.emit('handleMoved', i, newPosition);
            }
        }
        const queue = this.sendQueue[this.sendQueueIndex];
        if(this.serial && queue.length > 0) {
            serial.send(this.serial, queue.shift());
        }
    ...
    */
    ...
    /**
     * changes the panto PID factor
     * @param {Number} proportional the proportional factor
     * @param {Number} integral     the integral factor
     * @param {Number} derivative   the derivative factor
     */
    setPantoPID(proportional, integral, derivative) {
        this.emit('setPantoPID', proportional, integral, derivative);
        if(!this.serial)
            return;
        const values = [proportional, integral, derivative],
              packet = new Buffer(1+1+3*4);
        if(values.some(isNaN))
            return;
        packet[0] = 2; // control method : PIDSET
        packet[1] = 0; // Panto PID index
        packet.writeFloatLE(values[0], 2);
        packet.writeFloatLE(values[1], 6);
        packet.writeFloatLE(values[2], 10);
        this.send(0, packet); //Just send to upper handle
    }

    /**
     * changes the handle PID factor
     * @param {Number} proportional the proportional factor
     * @param {Number} integral     the integral factor
     * @param {Number} derivative   the derivative factor
     */
    setHandlePID(proportional, integral, derivative) {
        this.emit('setHandlePID', proportional, integral, derivative);
        if(!this.serial)
            return;
        const values = [proportional, integral, derivative],
              packet = new Buffer(1+1+3*4);
     ...
     */
    movePantoTo(index, target, speed = 120, interpolation_method = TWEEN.Easing.Quadratic.InOut) {
        if(speed > 120){
            console.log(speed, ' is too fast! Please keep your motors save with a maximum speed of 120mm/s!');
            speed = 120;
        }
     ...
     * @param {Object} [interpolation_method=TWEEN.Easing.Quadratic.InOut] - tween function that is used to generate the movement.
     */
    tweenPantoTo(index, target, duration = 500, interpolation_method = TWEEN.Easing.Quadratic.InOut) {
