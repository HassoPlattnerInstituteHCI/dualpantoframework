...
const ForceFieldHandler = require('./forcefieldHandler');
const Forcefield = require('./forcefield');
const Triggerfield = require('./triggerfield');
const Soundfield = require('./soundfield');
...
class Device extends EventEmitter {
  ...
  constructor(port) {
    ...
    this.meForceFieldhandler = new ForceFieldHandler();
    this.itForceFieldhandler = new ForceFieldHandler();
  }
  ...
  poll() {
    ...
    if (false && time[0] > this.lastReceiveTime[0] + broker.disconnectTimeout) {
  ...
  /**
   * handles god object position sent from the panto
   * @param {array} positions the god object positions sent by the panto
   */
  godObjectCallback(positions) {
    const meForce = this.meForceFieldhandler.updatePosition(positions[0],
        this.getMePosition());
    if (meForce.length() > 0) {
      meForced = true;
      this.applyForceTo(0, meForce);
    } else if (meForced) {
      this.unblock(0);
      meForced = false;
    }
    const itForce = this.itForceFieldhandler.updatePosition(positions[1],
        this.getItPosition());
    if (itForce.length() > 0) {
      itForced = true;
      this.applyForceTo(1, itForce);
    } else if (itForced) {
      this.unblock(1);
      itForced = false;
    }
  ...
  sendCreateObstacle(pointArray, id, index) {
    ...
    const pointArrayCopy = pointArray.slice();
  ...
  createForcefield(pointArray, handleFunction, index = -1) {
    ...
    if (index == -1) {
      this.meForceFieldhandler.addForcefield(forcefield);
      this.itForceFieldhandler.addForcefield(forcefield);
    } else if (index == 0) {
      this.meForceFieldhandler.addForcefield(forcefield);
    } else if (index == 1) {
      this.itForceFieldhandler.addForcefield(forcefield);
  ...
  removeForcefield(forcefield, index = -1) {
    if (index == -1) {
      this.meForceFieldhandler.removeForcefield(forcefield);
      this.itForceFieldhandler.removeForcefield(forcefield);
    } else if (index == 0) {
      this.meForceFieldhandler.removeForcefield(forcefield);
    } else if (index == 1) {
      this.itForceFieldhandler.removeForcefield(forcefield);
  ...
  /**
   * Creates triggerfields for handles
   * @param {array} pointArray - array containing edge points
   * of the triggerfield
   * @param {number} id - id of the obstacles to link
   * @param {number} [index =-1] - index of affected handle with -1 meaning both
   */
  createTriggerfield(pointArray, id, index = -1) {
    const triggerfield = new Triggerfield(pointArray, id);
    if (index == -1) {
      this.meForceFieldhandler.addTriggerField(triggerfield);
      this.itForceFieldhandler.addTriggerField(triggerfield);
    } else if (index == 0) {
      this.meForceFieldhandler.addTriggerField(triggerfield);
    } else if (index == 1) {
      this.itForceFieldhandler.addTriggerField(triggerfield);
    }
    triggerfield.on('disable!', function(obsID) {
      this.disableObstacle(obsID, index);
      setTimeout(() => this.enableObstacle(obsID, index), 5);
    }.bind(this));
  }

  /**
   * Creates soundfield for handles
   * @param {array} pointArray - array containing edge points
   * of the soundfield
   * @param {number} [index =-1] - index of affected handle with -1 meaning both
   * @return {Soundfield} the created soundfield
   */
  createSoundfield(pointArray, index = -1) {
    const soundfield = new Soundfield(pointArray);
    if (index == -1) {
      this.meForceFieldhandler.addSoundfield(soundfield);
      this.itForceFieldhandler.addSoundfield(soundfield);
    } else if (index == 0) {
      this.meForceFieldhandler.addSoundfield(soundfield);
    } else if (index == 1) {
      this.itForceFieldhandler.addSoundfield(soundfield);
