...
class Device extends EventEmitter {
  ...
  positionCallback(positions) {
    ...
    for (let i = 0; i < 2; ++i) {
      ...
      if (
        this.lastKnownPositions[i] &&
        positions[i * 2]
            .difference(this.lastKnownPositions[i])
            .length()
            <= minDist &&
        Math.abs(
            positions[i * 2].r - this.lastKnownPositions[i].r)
            <= minAngle) {
        continue;
      }
  ...
  /**
   * @description Requests new calibration of panto
   */
  ...
  /**
   * @description Requests Panto to move to calibrated starting position
   */
  sendStartingPositionRequest() {
    this.send({
      messageType: 0xB1
    });
  }
  ...
  movePantoTo(
    ...
    if (!this.lastKnownPositions[index]) {
      return new Promise((resolve) =>{
        setTimeout(() => {
          this.movePantoTo(index, target, speed, interpolation_method)
    ...
    return new Promise((resolve) => {
      ...
      const listener = new PositionListener(this, index, target, false);
      this.tweenPantoTo(index, target, listener, duration,
   ...
   * @param {number} index - Index of handle to move.
   * @param {Vector} target - Position the handle should be moved to.
   * @param {PositionListener} listener - Position listener for the target.
