...
class Device extends EventEmitter {
    ...
    poll() {
        ...
        if(this.serial && this.sendQueue[this.sendQueueIndex].length > 0)
            serial.send(this.serial, this.sendQueue[this.sendQueueIndex][this.sendQueue.length-1]);
        this.sendQueue[this.sendQueueIndex] = [];
    ...
    */
    createObstacle(pointArray, index=-1){
      if(index == -1){
        this.me.addObstacle(pointArray);
        this.it.addObstacle(pointArray);
      }else if(index == 0){
        this.me.addObstacle(pointArray);
      }else if(index == 1){
        this.it.addObstacle(pointArray);
     ...
     * @param {number} [index =-1] - index of affected handle with -1 meaning both
    */
    removeObstacle(pointArraym index =-1){
      if(index == -1){
        this.me.removeObstacle(pointArray);
        this.it.removeObstacle(pointArray);
      }else if(index == 0){
        this.me.removeObstacle(pointArray);
      }else if(index == 1){
        this.it.removeObstacle(pointArray);
    ...
    */
    step(){
      for(let i = 0; i < 2; i++){
        ...
        let difference = this.lastKnownPositions[i].difference(object.position);
        object.setMovementDirection(difference);
        object.move();
        if(object.processingCollision){
          let force = object.position.difference(this.lastKnownPositions[i]).scale(0.1125);
