...
class PositionListener extends EventEmitter {
   ...
   */
  constructor(device, index, target) {
    super();
    this.device = device;
    this.index = index;
    this.target = target;
  ...
  startListening() {
    if (this.index == 0) {
      ...
      this.device.on('handleMoved', this.callback);
  ...
  listenForMe(index, position) {
    if (this.index == 0 && position.difference(this.target).length() <= 2) {
  ...
  listenForIt(index, position) {
    ...
    if (this.index == 1 && position.difference(this.target).length() <= 2) {
      console.log('emit');
      this.device.removeListener('handleMoved', this.callback);
      this.emit('positionReached');
    }
