...
class Vector {
	 ...
	 * @param {number} angle the vector angle
	 ...
	 * @param {Vector|any} obj the object to test
	 * @returns {bool} whether obj is a vector
	 */
	static isVector(obj) {
		// faster then `obj instanceof Vector`
		return obj[isVector] === isVector;
	}

	/**
	 * Implement a nice debug string representation for `console.log(vector)`.
	 ...
	 * @param {Vector} a one end point
	 * @param {Vector} b the other end point
...
Vector.prototype[isVector] = isVector;

const immutabilityError = (oldName, newName, args = '') => () => {
	throw new Error(`Vector is now immutable: \`vector.${oldName}(${args})\` can not be used anymore: use \`vector = vector.${newName}(${args})\` instead`);
};

const mutatingMethods = {
	scale: immutabilityError('scale', 'scaled', 'factor'),
	add: immutabilityError('add', 'sum', 'other'),
	subtract: immutabilityError('subtract', 'diff', 'other'),
};

for(const key of Object.keys(mutatingMethods)) {
	mutatingMethods[key] = {
		configuarable: true,
		value: mutatingMethods[key],
	};
}
Object.defineProperties(Vector.prototype, mutatingMethods);
