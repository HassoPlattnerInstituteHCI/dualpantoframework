 ...
 * @prop {Map<string, Device>} deviceMap map of connected devices
 * @prop {Set<Device>} physicalDevices set of connected physical devices
 * @prop {number} defaultSpeed default speed for movements (overwrites duration)
 * @prop {number} defaultDuration default duration for movements
 * @prop {Object} defaultInterpolationMethod default interpolation method for movements
 * @prop {Object} defaultLanguage default language for spoken text
 * @prop {Object} defaultSpeechSpeed default speed for spoken text
 ...
 * @param {string} [options.defaultLanguage='DE'] default language for spoken text
 * @param {number} [options.defaultSpeechSpeed=1.4] default speed for spoken text
 * @param {Object|false} [options.visualDebugger={}] visual debugger options (false = disable)
 * @param {number} [options.visualDebugger.port=8080] visual debugger port
 * @param {bool} [options.deviceAutodetection=true] enable device autodetection
 */
class Broker extends EventEmitter {
	 ...
	 * @deprecated __Use `device.speakText(text)` instead!__
	 * @type {VoiceInteraction}
	 */
	get voiceInteraction() {
		deprecated('broker.voiceInteraction', 'device.speakText(text) and others');
		return this._voiceInteraction;
	}

	/**
	 * Creates a script that executes a list of promises.
	 * @deprecated __Use async / await instead!__
	 ...
	 * @deprecated __Use `delay(ms)` instead!__
	 ...
	 * @deprecated __Use `broker.devices` instead!__
	 * @returns {Iterator<Device>} the device iterator
	 * @private
	 */
	getDevices() {
	...
	/**
	 * Siganls that a new device is ready.
	 * Used by the {@link Device} once the configuration is loaded.
	 * @param {Device} device the device that is ready
	 * @private
	 */
	deviceReady(device) {
		console.log('dual panto: ready:', device.port);
