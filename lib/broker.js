...
const {deprecated, delay, immidiateDelay} = require('./util');
...
class Broker extends EventEmitter {
	...
	async serialRecv() {
		const counter = (this.serialRecvCounter + 1) | 0;
		this.serialRecvCounter = counter;

		while(this.serialRecvCounter === counter) {
			await immidiateDelay();
	...
	async autoDetectDevices() {
		...
		for(const port of ports) {
			if(isDualPanto(port)) {
				const normPort = normalizePort(port.comName);
				if(!this.deviceMap.has(normPort))
					this.createDevice(normPort);
			}
		}
	}
