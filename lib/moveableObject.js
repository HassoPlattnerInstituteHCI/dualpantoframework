...
class MoveableObject {
  ...
  move(){
    ...
    while(collisions.length > 0){
      const collidingEdge = collisions[0][1];//.getCollisionEdge(targetpoint, this.position);
      this.processingCollision = true;
      let outsidepoint = collisions[0][0].getClosestOutsidePoint(collidingEdge, targetpoint);
  ...
  checkCollison(point){
    ...
    for(let i = 0; i < this.obstacles.length; i++){
      ...
      if(information[0]){
        const collidingEdge = information[1].getCollisionEdge(point, this.position);
