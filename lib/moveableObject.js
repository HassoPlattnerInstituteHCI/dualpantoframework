...
class MoveableObject{
  ...
  move(){
    ...
    if(this.processingObstacleCollision){
      this.activeForce = this.position.difference(handlePosition).scale(smallPantoForceScale);
    }else{
  ...
  checkObstaclesCollison(point){
    ...
    for(let i = 0; i < this.obstacles.length; i++){
      const collidingEdge = this.obstacles[i].getEnteringEdge(point, this.position);
      if(collidingEdge!= null){
        collisions.push([this.obstacles[i], collidingEdge]);
      }
    }
    return collisions;
  }
