...
const start = ()=> {
  VoiceInteraction.setCommands(['Wohnungen', 'Kreuzberg', 'Mitte', 'Tempelhof', 'Berlin','Preis hoch', 'Preis runter', 'mehr Räume', 'weniger Räume', 'Keller ist notwendig', 'kein Keller', 'Größe anheben', 'Größe verringern', 'Preis ist egal', 'Größe ist egal', 'Keller ist egal', 'Anzahl der Räume ist egal', 'Hilfe']);
  ...
  DualPantoFramework.run_script([
    ...
    () => DualPantoFramework.waitMS(500),
    () => VoiceInteraction.speakText('Du kannst Wohnungen sagen und ich zeige dir alle Wohnungen für deine aktiven Filter'),
    () => DualPantoFramework.waitMS(500),
    () => VoiceInteraction.speakText('Es gibt verschiedene Filter: Preis, Anzahl der Räume, Keller vorhanden undGröße'),
    () => DualPantoFramework.waitMS(500),
    () => VoiceInteraction.speakText('Um den Preisfilter zu aktivieren musst du einen Maximalpreis setzen.'),
    () => DualPantoFramework.waitMS(500),
    () => VoiceInteraction.speakText('Mit dem Kommando Preis hoch oder Preis runter, kannst du den Preisfilter aktivieren und gleichzeitig den Preis anpassen.'),
    () => DualPantoFramework.waitMS(500),
    () => VoiceInteraction.speakText('Möchtest du den Preisfilter deaktivieren, sage Preis ist egal.'),
    () => DualPantoFramework.waitMS(500),
    () => VoiceInteraction.speakText('Um den Raumfilter zu aktivieren musst du einen Mindestanzahl an Räumen setzen.'),
    () => DualPantoFramework.waitMS(500),
    () => VoiceInteraction.speakText('Mit dem Kommando mehr Räume oder weniger Räume, kannst du den Raumfilter aktivieren und gleichzeitig die Mindestanzahl an Räumen anpassen.'),
    () => DualPantoFramework.waitMS(500),
    () => VoiceInteraction.speakText('Möchtest du den Raumfilter deaktivieren, sage Anzahl der Räume ist egal.'),
    () => DualPantoFramework.waitMS(500),
    () => VoiceInteraction.speakText('Um den Kellerfilter zu aktivieren musst du angeben, ob du einen Keller möchtest oder nicht.'),
    () => DualPantoFramework.waitMS(500),
    () => VoiceInteraction.speakText('Mit dem Kommando Keller ist notwendig oder kein Keller, setzt du deine Präferenz und aktivierst den Keller.'),
    () => DualPantoFramework.waitMS(500),
    () => VoiceInteraction.speakText('Möchtest du den Kellerfilter deaktivieren, sage Keller ist egal.'),
    () => DualPantoFramework.waitMS(500),
    () => VoiceInteraction.speakText('Um den Größenfilter zu aktivieren musst du eine Mindestgröße setzen.'),
    () => DualPantoFramework.waitMS(500),
    () => VoiceInteraction.speakText('Mit dem Kommando Größe anheben oder Größe verringern, kannst du den Größenfilter aktivieren und gleichzeitig die Mindestgröße anpassen.'),
    () => DualPantoFramework.waitMS(500),
    () => VoiceInteraction.speakText('Möchtest du den Größenfilter deaktivieren, sage Größe ist egal.'),
    () => DualPantoFramework.waitMS(500),
    () => VoiceInteraction.speakText('Wenn du Hilfe sagst, bekommst du nocheinmal die Einleitung zu den Filtern erklärt.'),
    () => DualPantoFramework.waitMS(500),
...
const showApartments = ()=> {
  ...
  for(const apartment of apartments){
    if(apartment.matchFilters()){
      ...
      script.push(() => device.movePantoTo(1, apartment.cords));
      script.push(() => DualPantoFramework.waitMS(500));
      script.push(() => VoiceInteraction.speakText('ist eine Wohnung in ' + apartment.area));
      script.push(() => DualPantoFramework.waitMS(500));
      script.push(() => VoiceInteraction.speakText('Die Wohnung Kostet ' + apartment.price + ' Euro im Monat.'));
      script.push(() => DualPantoFramework.waitMS(500));
      script.push(() => VoiceInteraction.speakText('Die Wohnung ist ' + apartment.size + ' Quadratmeter groß.'));
      script.push(() => DualPantoFramework.waitMS(500));
      script.push(() => VoiceInteraction.speakText('Die Wohnung hat ' + apartment.amountRooms + ' Räume.'));
      script.push(() => DualPantoFramework.waitMS(500));
      script.push(() => VoiceInteraction.speakText('Die Wohnung hat' + hasCellar + ' Keller.'));
      script.push(() => DualPantoFramework.waitMS(500));
    }
  }
